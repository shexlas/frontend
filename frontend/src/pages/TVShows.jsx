...
export default function TVShows() {
  ...
  const load = useCallback(() => {
    ...
    api.get(`/series?${query.toString()}`)
      ...
      .catch((err) => setError(errorMessage(err, 'Could not load TV shows.')));
  }, [sort, genre]);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="text-2xl font-bold">
          {genre ? `${genre} TV Shows` : sort === 'rating' ? 'Top 10 TV Shows' : sort === 'popular' ? 'Popular TV Shows' : 'All TV Shows'}
        </h1>
      ...
      {series?.length === 0 && <p className="text-gray-400">No TV shows found.</p>}
